...
# 执行自定义任务，执行对象是所有
# ansible2.run(hosts= "all", module="shell", args='uptime')
ansible2.run(hosts="all", module="setup", args='')
# 打印结果
# data = ansible2.get_result()
data = json.loads(ansible2.get_result())
# print(data)
print(data['success']['192.168.205.130']['ansible_facts']['ansible_processor_vcpus'])
print(data['success']['192.168.205.130']['ansible_facts']['ansible_memory_mb']['real']['total'])

# 执行计划任务
# ansible2 = MyAnsiable2(inventory='/etc/ansible/hosts', connection='smart')
#
# # 传入playbooks 的参数，需要是一个列表的数据类型，这里是使用的相对路径
# ansible2.playbook(playbooks=['tmp_playbook.yml'])
# ansible2.get_result()
